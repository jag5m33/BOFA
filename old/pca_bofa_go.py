...
# ================================================================
# SECTION 2: PCA training
# ================================================================
pca = pca_mod.build_pca(n_components=4)

# Train ONLY on athletes
pca = pca_mod.train_pca(pca, df_tensor, sources)

# elbow plot of PCA variance results
pca_mod.plot_pca_vairance(pca)


# Transform ALL data
latent_data_raw = pca.transform(df_tensor)

# Create column names
latent_cols = [f"latent_{i}" for i in range(latent_data_raw.shape[1])]
features = latent_cols

# Reconstruction error (PCA version)
reconstructed = pca.inverse_transform(latent_data_raw)
recon_error_matrix = np.square(df_tensor - reconstructed)
...
# ================================================================
latent_df = pd.DataFrame(latent_data_raw, columns=latent_cols)
